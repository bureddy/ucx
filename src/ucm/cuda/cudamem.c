...
extern ucs_status_t (*ucm_mem_type_get_current_device_info[UCS_MEMORY_TYPE_LAST])(int *device_id, ucs_sys_bus_id_t *bus_id);
...
static int uct_ib_iface_cuda_sysfs_path(int cuda_dev, ucs_sys_bus_id_t *bus_id)
{
    /* TODO : Find Bus ID */
    memset(bus_id, 0, sizeof(ucs_sys_bus_id_t));

    return UCS_OK;
}

ucs_status_t ucm_cuda_get_current_device_info(int *device_id, ucs_sys_bus_id_t *bus_id)
{
    CUresult cu_err;
    ...
    uct_ib_iface_cuda_sysfs_path(cuda_device, bus_id);
    *device_id = (int)cuda_device;
    ucm_debug("device id:%d", *device_id);
