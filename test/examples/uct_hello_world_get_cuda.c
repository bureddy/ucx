...
/* Helper data type for get_bcopy */
typedef struct {
    char               *data;
    size_t              len;
} get_bcopy_args_t;
...
static char *func_get_t_str(func_get_t func_get_type)
{
    switch (func_get_type) {
    case FUNC_GET_BCOPY:
        return "uct_ep_get_bcopy";
    case FUNC_GET_ZCOPY:
        return "uct_ep_get_zcopy";
    }
    return NULL;
}

static size_t func_get_max_size(func_get_t func_get_type,
                               const uct_iface_attr_t *attr)
{
    switch (func_get_type) {
    case FUNC_GET_BCOPY:
        return attr->cap.get.max_bcopy;
    case FUNC_GET_ZCOPY:
        return attr->cap.get.max_zcopy;
    }
    return 0;
}

/* Pack callback for get_bcopy */
void get_bcopy_data_unpack_cb(void *dest, const void *src, size_t length)
{
    memcpy(dest, src, length);
    return;
}

void bcopy_completion_cb(uct_completion_t *self, ucs_status_t status)
{
    uct_completion_t *uct_comp = self;
    assert(((*uct_comp).count == 0) && (status == UCS_OK));
}

/* Executed by the client alone */
ucs_status_t do_get_bcopy(uct_ep_h ep, const cmd_args_t *cmd_args, char *buf,
                          ...
                          iface_info_t *if_info)
{
    ...
    uct_comp.func = bcopy_completion_cb;
    uct_comp.count = 1;

    status = uct_ep_get_bcopy(ep, get_bcopy_data_unpack_cb, buf,
                              cmd_args->test_strlen, peer_addr, rkey,
                              &uct_comp);
    if (status == UCS_INPROGRESS) {
        while (0 != uct_comp.count) {
            /* Explicitly progress outstanding get request */
...
/* Completion callback for get_zcopy */
...
ucs_status_t do_get_zcopy(uct_ep_h ep, const cmd_args_t *cmd_args, char *buf,
                          ...
                          iface_info_t *if_info)
{
    ...
    status = uct_ep_get_zcopy(ep, &iov, iovcnt, peer_addr, rkey, &uct_comp);
    if (status == UCS_INPROGRESS) {
        while (0 != uct_comp.count) {
            /* Explicitly progress outstanding get request */
            uct_worker_progress(if_info->worker);
                               ...
                               iface_info_t *iface_p)
{
    ...
    if ((func_get_type == FUNC_GET_BCOPY) &&
        (iface_p->attr.cap.flags & UCT_IFACE_FLAG_GET_BCOPY)) {
        return UCS_OK;
    }

    if ((func_get_type == FUNC_GET_ZCOPY) &&
        (iface_p->attr.cap.flags & UCT_IFACE_FLAG_GET_ZCOPY)) {
        return UCS_OK;
    }
        ...
        for (j = 0; j < num_tl_resources; ++j) {
            if (!strcmp(cmd_args->dev_name, tl_resources[j].dev_name) &&
                ...
                status = init_iface(tl_resources[j].dev_name,
                                    ...
                                    cmd_args->func_get_type, iface_p);
                if (UCS_OK == status) {
...
int print_err_usage()
{
    ...
    fprintf(stderr, "\nParameters are:\n");
    fprintf(stderr, func_template, 'b', func_get_t_str(FUNC_GET_BCOPY), " (default)");
    fprintf(stderr, func_template, 'z', func_get_t_str(FUNC_GET_ZCOPY), "");
...
int parse_cmd(int argc, char * const argv[], cmd_args_t *args)
{
    ...
    args->func_get_type  = FUNC_GET_BCOPY;
    args->test_strlen   = 16;

    opterr = 0;
    while ((c = getopt(argc, argv, "bzd:t:n:p:s:h")) != -1) {
        switch (c) {
        case 'b':
            args->func_get_type = FUNC_GET_BCOPY;
            break;
        case 'z':
            args->func_get_type = FUNC_GET_ZCOPY;
            break;
        case 'd':
    ...
    fprintf(stderr, "INFO: UCT_HELLO_WORLD AM function = %s server = %s port = %d\n",
            func_get_t_str(args->func_get_type), args->server_name,
            args->server_port);
...
int main(int argc, char **argv)
{
    ...
    if (cmd_args.test_strlen > func_get_max_size(cmd_args.func_get_type,
                                                &if_info.attr)) {
        status = UCS_ERR_UNSUPPORTED;
        fprintf(stderr, "Test string is too long: %ld, max supported: %lu\n",
                cmd_args.test_strlen,
                func_get_max_size(cmd_args.func_get_type, &if_info.attr));
        goto out_free_ep;
    }
    ...
    /* At this point rkey and remote address is available for get operations */

    if (cmd_args.server_name) {

        /* Invoke get on remote endpoint */
        if (cmd_args.func_get_type == FUNC_GET_ZCOPY) {
            status = do_get_zcopy(ep, &cmd_args, str, &memh_buf, *peer_addr,
                                  rkey_ob.rkey, &if_info);
        }
        else if (cmd_args.func_get_type == FUNC_GET_BCOPY) {
            status = do_get_bcopy(ep, &cmd_args, str, *peer_addr, rkey_ob.rkey,
                                  &if_info);
            while (str[cmd_args.test_strlen - 1] == 'a') {
                /* wait */
            }
        }
        CHKERR_JUMP(UCS_OK != status, "get op", out_free_ep);
    } else {
        if (cmd_args.func_get_type == FUNC_GET_ZCOPY ||
            cmd_args.func_get_type == FUNC_GET_BCOPY) {
            /* do nothing */
        }
    }
