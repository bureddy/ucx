...
void test_md::init()
{
    ...
    /* Set context to allow device allocations through uct_mem_alloc */


    for (std::vector<ucs_memory_type_t>::const_iterator
         iter = supported_mem_types.begin();
         iter != supported_mem_types.end(); ++iter) {
         if (m_md_attr.cap.alloc_mem_types & UCS_BIT(*iter)) {
            if ((*iter == UCS_MEMORY_TYPE_CUDA) ||
                (*iter == UCS_MEMORY_TYPE_CUDA_MANAGED)) {
                void *address = mem_buffer::allocate(1, (ucs_memory_type_t)*iter);
                mem_buffer::release(address, (ucs_memory_type_t)*iter);
            }
        }
    }
